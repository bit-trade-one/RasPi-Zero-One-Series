...
class GYRO():
	...
	def read_loop(self):
		while True:
			...
			self.__GYRO_X = self.gyro_conv((Sense_return[8]<<8) + Sense_return[9]) + self.__calib_x
			self.__GYRO_Y = self.gyro_conv((Sense_return[10]<<8) + Sense_return[11]) + self.__calib_y
			self.__GYRO_Z = self.gyro_conv((Sense_return[12]<<8) + Sense_return[13]) + self.__calib_z
	...
	def accel_conv(self,val):
		return (struct.unpack("<h",struct.pack("<H",val))[0]) * (2 ** (self.__ac_conf + 1))/32768

	def gyro_conv(self,val):
		return (struct.unpack("<h",struct.pack("<H",val))[0]) *250* (2 ** self.__gr_conf)/32768
	...
	def get_sense_value(self):
		if self.__INIT_FLAG:
			return self.__ACCEL_X,self.__ACCEL_Y,self.__ACCEL_Z,self.__GYRO_X,self.__GYRO_Y,self.__GYRO_Z,self.__MAG_X,self.__MAG_Y,self.__MAG_Z
		else:
			return None
	...
	def sensor_calib(self,count = 10):
		...
		print("\nキャリブレーション終了")
		print("X:{0}\tY:{1}\tZ:{2}\n".format(self.__calib_x,self.__calib_y,self.__calib_z))
