...
#-------------------------------------------------------------------------------------------------------------------------------------
	...
	def Get_servo_duty(self,ch):
		if ch == "1":
			return struct.unpack(">H",struct.pack("<H",i2c.read_word_data(self.__addr,self.GET_CH1_DUTY)))[0]
		elif ch == "2":
			return struct.unpack(">H",struct.pack("<H",i2c.read_word_data(self.__addr,self.GET_CH2_DUTY)))[0]
		else:
			print("invalid command")

	def Get_servo_duty_isolate(self):
		print("CH1:{0}".format(struct.unpack(">H",struct.pack("<H",i2c.read_word_data(self.__addr,self.GET_CH1_DUTY)))[0]))
		print("CH2:{0}".format(struct.unpack(">H",struct.pack("<H",i2c.read_word_data(self.__addr,self.GET_CH2_DUTY)))[0]))

	def Get_servo_duty_max(self,ch):
		if ch == "1":
			return struct.unpack(">H",struct.pack("<H",i2c.read_word_data(self.__addr,self.GET_CH1_DUTY_MAX)))[0]
		elif ch == "2":
			return struct.unpack(">H",struct.pack("<H",i2c.read_word_data(self.__addr,self.GET_CH2_DUTY_MAX)))[0]
		else:
			print("invalid command")

	def Get_servo_duty_min(self,ch):
		if ch == "1":
			return struct.unpack(">H",struct.pack("<H",i2c.read_word_data(self.__addr,self.GET_CH1_DUTY_MIN)))[0]
		elif ch == "2":
			return struct.unpack(">H",struct.pack("<H",i2c.read_word_data(self.__addr,self.GET_CH2_DUTY_MIN)))[0]
		else:
			print("invalid command")

	def Get_servo_cycle(self):
		return struct.unpack(">H",struct.pack("<H",i2c.read_word_data(self.__addr,self.GET_PWM_CYCL)))[0]
...
#-------------------------------------------------------------------------------------------------------------------------------------
	...
	def Set_servo_duty_isolate(self,ch1_val_str,ch2_val_str):
		...
		i2c.write_i2c_block_data(self.__addr,self.CH1_DUTY,[(ch1_val//0x100),(ch1_val%0x100),(ch2_val//0x100),(ch2_val%0x100),0x03])
