...
def read_command():
    ...
    bus.write_byte(SLAVE_ADDRESS, 0x15 )   #= 0x15  #bus-write(ADR,cmd,1)OSError: [Errno 121] Remote I/O error  
...
# cmd R2_data_num_read 0x25 bus-read(ADR,cmd,3)
    data_numHL = bus.read_i2c_block_data(SLAVE_ADDRESS, R3_data_num_read ,3 )#= 0x25 #bus-read(ADR,cmd,3)
...
# cmd R3_data_read           0x35 bus-read(ADR,cmd,n)
     block = []
     block_dat  = bus.read_i2c_block_data(SLAVE_ADDRESS, R4_data_read , 1)       #= 0x35 #bus-read(ADR,cmd,n)
     for i in range(data_num ):
      block_dat  = bus.read_i2c_block_data(SLAVE_ADDRESS, R4_data_read , 4)       #= 0x35 #bus-read(ADR,cmd,n)
